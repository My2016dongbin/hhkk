...
class DeviceDetailController extends GetxController {
  ...
  Future<void> getSecondPlayUrl() async {
    dynamic params = {
    ...
    if (result["code"] == 0 && result["data"] != null) {
      try {
        final dynamic playPath =
            result["data"]["rtspStreamPath"] ?? result["data"]["fullStreamPath"] ?? '';
        String url = "$playPath";
        if (url.isEmpty) {
  ...
  void startFijkPlayUrl(String url) {
    playLoadingTag.value = false;
    playTag.value = false;
    player.release();
    player = FijkPlayer();
