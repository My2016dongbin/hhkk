...
struct HPR_NETWORK_FLOWDATA
{
	...
	HPR_UINT32 nReserve[4]; //nReserve[0]:网卡速率in Mbps,（解决万兆时nSpeed会超HPR_UINT32最大值  addd 2019-03-02）
};

struct HPR_NETWORK_FLOWDATA_EX
{
    char szAdapterName[260];	//[in] get from HPR_ADAPTER_INFO.szAdapterName {req in linux}
    HPR_INT32 iIndex;	//[in] get from HPR_ADAPTER_INFO.iIndex
    HPR_UINT64 nSpeed;	//in bps
    HPR_UINT64 nInBytes;
    HPR_UINT64 nInPackets;
    HPR_UINT64 nOutBytes;
    HPR_UINT64 nOutPackets;
...
HPR_DECLARE HPR_INT32 CALLBACK HPR_GetNetWorkFlowDataEx(HPR_NETWORK_FLOWDATA_EX *flowdata);
#endif
